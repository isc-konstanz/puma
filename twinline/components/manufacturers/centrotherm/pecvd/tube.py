# -*- coding: utf-8 -*-
"""
twinline.components.machines.depositors.pecvd.tube
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
from twinline.components.machines.depositors import VacuumTube
from twinline.components.manufacturers.centrotherm.pecvd import CPlasmaZone
...
class CPlasmaTube(VacuumTube):
    ...
    def _load_zones(self, configs: Configurations, number: int) -> Collection[CPlasmaZone]:
        defaults = CPlasmaZone._build_defaults(configs, strict=True)
        zones = []

        def _add_zone(index: int, key: str, name: str) -> None:
            zone = self._load_zone(configs.get_section(str(key), defaults=defaults), index, key, name)
            zones.append(zone)
            self.components.add(zone)

        # fmt: off
        _add_zone(1, key="lz",        name="Load")
        _add_zone(2, key="center_lz", name="Center Load")
        _add_zone(3, key="center",    name="Center")
        _add_zone(4, key="center_gz", name="Center Gas")
        _add_zone(5, key="gz",        name="Gas")
        # fmt: on
